...
- 标记为广告：点击或框选自动识别广告元素（链接 / 图片 / 区块），读取其链接与区块代码并记录，自动元素隐藏 + 广告主机拦截，可在“广告拦截”中开关、删除
...
- 隐私与安全：
  ...
  - 广告拦截（内置基础 / 严格两级列表 + AdGuard DNS 过滤主机列表，16 万+ 广告/追踪域名）+ 用户自定义屏蔽规则
...
.\gradlew.bat :app:testDebugUnitTest    # 单元测试（261 个用例）
```

产物位于 `app/build/outputs/apk/`；已签名 Release 位于 `app/build/outputs/apk/release/app-release.apk`。
...
内置广告拦截规则包含 AdGuard DNS filter（CC BY-SA 3.0，https://github.com/AdguardTeam/AdguardSDNSFilter）。
